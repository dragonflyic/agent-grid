...
class Planner:
    ...
    async def decompose(self, repo: str, issue_number: int, title: str, body: str, author: str = "") -> list[dict]:
        ...
        """
        # Label as planning
        await self._labels.transition_to(repo, str(issue_number), "ag/planning")
        ...
        try:
            ...
            await self._labels.transition_to(repo, str(issue_number), "ag/failed")
...
_Auto-generated by Tech Lead Agent from #{issue_number}_"""
            ...
            # Assign sub-issue to parent author if known
            if author:
                try:
                    await self._tracker.assign_issue(repo, str(sub_issue.number), author)
                except Exception as e:
                    logger.warning(f"Failed to assign sub-issue #{sub_issue.number} to {author}: {e}")
...
{chr(10).join(f"- {r}" for r in plan.get("risks", []))}"""
        ...
        await get_status_comment_manager().post_or_update_slot(repo, str(issue_number), "plan", plan_comment)

        # Label parent as epic (transition_to removes ag/planning atomically)
